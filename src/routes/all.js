...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        if (userId) {
            ...
            queryStr = `SELECT recipes.id, recipes.image, recipes.title, recipes.description, recipes.owner_id, COUNT(comments.id) AS comments_num FROM recipes LEFT JOIN comments ON recipes.id = comments.recipe_id GROUP BY recipes.id ORDER BY recipes.timestamp DESC LIMIT 30`;
        }
        const [recipesAll] = await con.execute(queryStr);
        con.end();
        res.send({ data: recipesAll });
    }
    catch(e){
        console.log(e);
